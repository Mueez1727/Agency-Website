...
export function ServiceCard({ service }: ServiceCardProps) {
  ...
  return (
    <article className="group flex h-full flex-col rounded-3xl border border-line bg-white p-6 shadow-[var(--shadow-card)] transition duration-300 hover:-translate-y-1 hover:shadow-[var(--shadow-lift)] sm:p-7">
      <div className="mb-5 inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-accent-soft text-accent">
      ...
      <div className="mt-6">
        <Button to={`/?service=${service.id}#contact`} variant="outline" className="w-full sm:w-auto">
          {service.cta}
